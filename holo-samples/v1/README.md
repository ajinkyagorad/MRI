...
## Corrected small-face geometry — viewer 1.34.1

`surface-caches-v5.json` replaces v4 geometry for new viewer versions. Very small triangle normals now retain a unit length when constructing quadrics; the prior Unity vector-normalization threshold could erase their curvature error. This improves shape retention at the same 160,000-triangle limit. The source NIfTI labels are unchanged. Legacy v4 blobs remain only for clients pinned to older data revisions; new clients use the v5 key, header and cache directory.
